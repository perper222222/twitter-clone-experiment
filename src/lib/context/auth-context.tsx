...
type AuthContext = {
  ...
  signInWithNumber: (id: number) => Promise<void>;
};
...
export function AuthContextProvider({
  ...
  const signInWithNumber = async (num: number): Promise<void> => {
    try {
      // Accept only 1-200
      if (!Number.isInteger(num) || num < 1 || num > 200) {
        throw new Error('Experiment ID must be an integer between 1 and 200');
      }
      const id = String(num);
      // Create or update user record in Firestore under users collection
      ...
      // Manually set user state to the created user (minimal fields)
      setUser({ id, username: id, name: `Participant ${id}`, bio: null, photoURL: null } as User);
    } catch (error) {
      setError(error as Error);
    }
  };
